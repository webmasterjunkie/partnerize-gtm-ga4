...

{
  ...
  "version": 1,
  "securityGroups": [],
  "displayName": "Partnerize GA4 eCommerce Tag",
  "categories": [
    "AFFILIATE_MARKETING",
    "ADVERTISING",
    "CONVERSIONS",
  ...
  "description": "This template provides support for GA 4 purchase tracking via recommended eCommerce structure.",
  "containerContexts": [
    "WEB"
  ]
}
...
[
  {
    ...
    "help": "This value would have been provided to you by your Partnerize integration engineer."
  },
  {
    "type": "CHECKBOX",
    "name": "VoucherCode",
    ...
    "simpleValueType": true,
    "help": "If you have a voucher code in your dataLayer and want to include it in the conversion call, check this box."
  },
  {
    "type": "LABEL",
    "name": "ParamDetails",
    "displayName": "If you would like to include data not included in the dataLayer you can add them here as conversion level parameters. The default tag will automatically include values for \"conversionref\", \"currency\", etc."
  },
  {
    "type": "SIMPLE_TABLE",
    "name": "CustomValues",
...
if (ecom.currency){
	url = url + 'currency:' + encodeUriComponent(ecom.currency) + '/';
}
...
if (data.VoucherCode){
	if (ecom){
		if (ecom.coupon){
			url = url + 'voucher:' + encodeUriComponent(ecom.coupon) + '/';
		}
	}
}
...
if (ecom){
	if (ecom.items){
		url = url + createItemString(ecom.items);
	}
}
	...
	for (var i = 0; i < ecomProducts.length; i++){
		...
		for (var key in ecomProducts[i]){
			switch (key){
				...
				default:
					if (ecomProducts[i][key]) {
					    stringproducts = stringproducts + formatKey(key) + ':' + encodeUriComponent(ecomProducts[i][key].toString()) + '/';
					} else {
					    stringproducts = stringproducts + formatKey(key) + ':/';
					}
				break;
			}
		}
...
function formatKey(key){
  var pretty = key.replace(' ', '_').toLowerCase();
  
  if (pretty.slice(0, 5) == 'item_'){
    return pretty.slice(5);
  } else {
    return pretty;
  }
}
...
Created on 11/22/2022, 1:15:15 PM
